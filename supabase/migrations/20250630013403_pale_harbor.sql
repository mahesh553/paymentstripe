...
-- Create RLS policies for xxpj_subscriptions (with IF NOT EXISTS check)
DO $$
BEGIN
  IF NOT EXISTS (
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'xxpj_subscriptions' AND policyname = 'Users can update own xxpj_subscriptions'
  ) THEN
    CREATE POLICY IF NOT EXISTS "Users can update own xxpj_subscriptions"
      ON xxpj_subscriptions
      FOR UPDATE
      TO authenticated
      USING (auth.uid() = user_id);
...
-- Create RLS policies for xxpj_usage (with IF NOT EXISTS check)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'xxpj_usage' AND policyname = 'Users can read own xxpj_usage'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'xxpj_usage' AND policyname = 'Users can insert own xxpj_usage'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'xxpj_usage' AND policyname = 'Users can update own xxpj_usage'
  ) THEN
...
-- Create triggers for updated_at columns (with IF NOT EXISTS check)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.triggers 
    WHERE trigger_name = 'update_xxpj_subscriptions_updated_at'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.triggers 
    WHERE trigger_name = 'update_xxpj_usage_updated_at'
  ) THEN
