-- Resume Analyzer Database Schema - Final Clean Migration
...
-- Add unique constraints only if they don't exist - using pg_constraint for more reliable checking
DO $$
BEGIN
  -- Check and add unique constraint on xxpj_subscriptions.user_id
  IF NOT EXISTS (
  ...
  -- Check and add unique constraint on xxpj_subscriptions.stripe_customer_id
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint 
  ...
  -- Check and add unique constraint on xxpj_subscriptions.stripe_subscription_id
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint 
  ...
  -- Check and add unique constraint on xxpj_usage_tracking
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint 
  ...
  -- Check and add unique constraint on xxpj_usage.user_id - SKIP if already exists
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint 
  ...
  -- Check and add unique constraint on xxpj_feature_flags.flag_name
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint 
  ...
  -- Check and add unique constraint on xxpj_subscription_history.stripe_event_id
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint 
...
DO $$
BEGIN
  -- Drop all existing policies for clean slate - using a more comprehensive approach
  PERFORM 1 FROM pg_policies WHERE schemaname = 'public' AND tablename = 'xxpj_users';
  IF FOUND THEN
  ...
  PERFORM 1 FROM pg_policies WHERE schemaname = 'public' AND tablename = 'xxpj_subscriptions';
  IF FOUND THEN
    ...
    DROP POLICY IF EXISTS "Users can read own subscriptions" ON xxpj_subscriptions;
    DROP POLICY IF EXISTS "Users can update own subscriptions" ON xxpj_subscriptions;
  END IF;
  
  PERFORM 1 FROM pg_policies WHERE schemaname = 'public' AND tablename = 'xxpj_usage';
  IF FOUND THEN
    ...
    DROP POLICY IF EXISTS "Users can read own usage" ON xxpj_usage;
    DROP POLICY IF EXISTS "Users can insert own usage" ON xxpj_usage;
    DROP POLICY IF EXISTS "Users can update own usage" ON xxpj_usage;
  END IF;
  
  PERFORM 1 FROM pg_policies WHERE schemaname = 'public' AND tablename = 'xxpj_usage_tracking';
  IF FOUND THEN
    ...
    DROP POLICY IF EXISTS "Users can read own usage tracking" ON xxpj_usage_tracking;
    DROP POLICY IF EXISTS "Users can insert own usage tracking" ON xxpj_usage_tracking;
    DROP POLICY IF EXISTS "Users can update own usage tracking" ON xxpj_usage_tracking;
  END IF;
  
  PERFORM 1 FROM pg_policies WHERE schemaname = 'public' AND tablename = 'resumes';
  IF FOUND THEN
  ...
  PERFORM 1 FROM pg_policies WHERE schemaname = 'public' AND tablename = 'analyses';
  IF FOUND THEN
  ...
  PERFORM 1 FROM pg_policies WHERE schemaname = 'public' AND tablename = 'job_matches';
  IF FOUND THEN
...
-- Create RLS Policies
CREATE POLICY "Users can view their own profile" ON xxpj_users FOR SELECT TO authenticated USING (auth.uid() = id);
CREATE POLICY "Users can update their own profile" ON xxpj_users FOR UPDATE TO authenticated USING (auth.uid() = id);
CREATE POLICY "Users can insert their own profile" ON xxpj_users FOR INSERT TO authenticated WITH CHECK (auth.uid() = id);
